...
from src.models.csv_models import CsvProduct, CsvScore, ScoreComponent
...
class CsvParser:
    ...
    def _create_scores(self, row: pd.Series, start_index: int) -> List[CsvScore]:
        ...
        Returns:
            Liste des 3 scores (local, eco, living)
        """
        scores = []
    ...
    def parse_products(self) -> List[CsvProduct]:
        ...
        for _, row in self.df.iloc[3:].iterrows():
            ...
            product = CsvProduct(
                ...
                product_scores=self._create_scores(row, self.product_score_index),
                service_scores=self._create_scores(row, self.service_score_index)
            )
